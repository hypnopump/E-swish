...
def NASNet(act="relu",
    ...
    x, p = _reduction_A(act, x, None, filters // (filters_multiplier ** 2), id='stem_1')
    x, p = _reduction_A(act, x, p, filters // filters_multiplier, id='stem_2')

    for i in range(nb_blocks):
        x, p = _normal_A(act, x, p, filters, id='%d' % (i))

    x, p0 = _reduction_A(act, x, p, filters * filters_multiplier, id='reduce_%d' % (nb_blocks))

    p = p0 if not skip_reduction else p

    for i in range(nb_blocks):
        x, p = _normal_A(act, x, p, filters * filters_multiplier, id='%d' % (nb_blocks + i + 1))
    ...
    if use_auxilary_branch:
        ...
        with K.name_scope('auxilary_branch'):
            ...
            auxilary_x = Activation(act)(auxilary_x)
    ...
    x, p0 = _reduction_A(act, x, p, filters * filters_multiplier ** 2, id='reduce_%d' % (2 * nb_blocks))

    p = p0 if not skip_reduction else p

    for i in range(nb_blocks):
        x, p = _normal_A(act, x, p, filters * filters_multiplier ** 2, id='%d' % (2 * nb_blocks + i + 1))
...
def _separable_conv_block(act, ip, filters, kernel_size=(3, 3), strides=(1, 1), id=None):
...
def _adjust_block(act, p, ip, filters, id=None):
    '''
...
def _normal_A(act, ip, p, filters, id=None):
    ...
    with K.name_scope('normal_A_block_%s' % id):
        p = _adjust_block(act, p, ip, filters, id)
        ...
        with K.name_scope('block_1'):
            x1 = _separable_conv_block(act, h, filters, id='normal_left1_%s' % id)
            x1 = add([x1, h], name='normal_add_1_%s' % id)

        with K.name_scope('block_2'):
            x2_1 = _separable_conv_block(act, p, filters, id='normal_left2_%s' % id)
            x2_2 = _separable_conv_block(act, h, filters, kernel_size=(5, 5), id='normal_right2_%s' % id)
        ...
        with K.name_scope('block_5'):
            x5_1 = _separable_conv_block(act, p, filters, (5, 5), id='normal_left5_%s' % id)
            x5_2 = _separable_conv_block(act, p, filters, (3, 3), id='normal_right5_%s' % id)
...
def _reduction_A(act, ip, p, filters, id=None):
    ...
    with K.name_scope('reduction_A_block_%s' % id):
        ...
        with K.name_scope('block_1'):
            x1_1 = _separable_conv_block(act, p, filters, (7, 7), strides=(2, 2), id='reduction_left1_%s' % id)
            x1_2 = _separable_conv_block(act, h, filters, (5, 5), strides=(2, 2), id='reduction_right1_%s' % id)
            x1 = add([x1_1, x1_2], name='reduction_add_1_%s' % id)

        with K.name_scope('block_2'):
            x2_1 = MaxPooling2D((3, 3), strides=(2, 2), padding='same', name='reduction_left2_%s' % id)(h)
            x2_2 = _separable_conv_block(act, p, filters, (7, 7), strides=(2, 2), id='reduction_right2_%s' % id)
        ...
        with K.name_scope('block_3'):
            ...
            x3_2 = _separable_conv_block(act, p, filters, (5, 5), strides=(2, 2), id='reduction_right3_%s' % id)
            x3 = add([x3_1, x3_2], name='reduction_add3_%s' % id)

        with K.name_scope('block_4'):
            x4_1 = MaxPooling2D((3, 3), strides=(2, 2), padding='same', name='reduction_left4_%s' % id)(h)
            x4_2 = _separable_conv_block(act, x1, filters, (3, 3), id='reduction_right4_%s' % id)
...
if __name__ == '__main__':
    ...
    model = NASNetCIFAR(input_shape=(224, 224, 3))
