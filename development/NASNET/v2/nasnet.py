...
def NASNet(act="relu",
    ...
    if not skip_reduction:  # imagenet / mobile mode
        x, p = _reduction_A(act, x, p, filters // (filters_multiplier ** 2), weight_decay, id='stem_1')
        x, p = _reduction_A(act, x, p, filters // filters_multiplier, weight_decay, id='stem_2')

    for i in range(nb_blocks):
        x, p = _normal_A(act, x, p, filters, weight_decay, id='%d' % (i))

    x, p0 = _reduction_A(act, x, p, filters * filters_multiplier, weight_decay, id='reduce_%d' % (nb_blocks))

    p = p0 if not skip_reduction else p

    for i in range(nb_blocks):
        x, p = _normal_A(act, x, p, filters * filters_multiplier, weight_decay, id='%d' % (nb_blocks + i + 1))

    auxiliary_x = None
    if not skip_reduction:  # imagenet / mobile mode
        if use_auxiliary_branch:
            auxiliary_x = _add_auxiliary_head(act, x, classes, weight_decay)

    x, p0 = _reduction_A(act, x, p, filters * filters_multiplier ** 2, weight_decay, id='reduce_%d' % (2 * nb_blocks))
    ...
    for i in range(nb_blocks):
        x, p = _normal_A(act, x, p, filters * filters_multiplier ** 2, weight_decay, id='%d' % (2 * nb_blocks + i + 1))
...
def _separable_conv_block(act, ip, filters, kernel_size=(3, 3), strides=(1, 1), weight_decay=5e-5, id=None):
...
def _adjust_block(act, p, ip, filters, weight_decay=5e-5, id=None):
    '''
    ...
    with K.name_scope('adjust_block'):
        if p is None:
            ...
            with K.name_scope('adjust_reduction_block_%s' % id):
                p = Activation(act, name='adjust_relu_1_%s' % id)(p)
...
def _normal_A(act, ip, p, filters, weight_decay=5e-5, id=None):
    ...
    with K.name_scope('normal_A_block_%s' % id):
        p = _adjust_block(act, p, ip, filters, weight_decay, id)
        ...
        with K.name_scope('block_1'):
            x1_1 = _separable_conv_block(act, h, filters, kernel_size=(5, 5), weight_decay=weight_decay,
                                         id='normal_left1_%s' % id)
            x1_2 = _separable_conv_block(act, p, filters, weight_decay=weight_decay, id='normal_right1_%s' % id)
            x1 = add([x1_1, x1_2], name='normal_add_1_%s' % id)

        with K.name_scope('block_2'):
            x2_1 = _separable_conv_block(act, p, filters, (5, 5), weight_decay=weight_decay, id='normal_left2_%s' % id)
            x2_2 = _separable_conv_block(act, p, filters, (3, 3), weight_decay=weight_decay, id='normal_right2_%s' % id)
        ...
        with K.name_scope('block_5'):
            x5 = _separable_conv_block(act, h, filters, weight_decay=weight_decay, id='normal_left5_%s' % id)
...
def _reduction_A(act, ip, p, filters, weight_decay=5e-5, id=None):
    ...
    with K.name_scope('reduction_A_block_%s' % id):
        ...
        with K.name_scope('block_1'):
            x1_1 = _separable_conv_block(act, h, filters, (5, 5), strides=(2, 2), weight_decay=weight_decay,
                                         id='reduction_left1_%s' % id)
            x1_2 = _separable_conv_block(act, p, filters, (7, 7), strides=(2, 2), weight_decay=weight_decay,
        ...
        with K.name_scope('block_2'):
            ...
            x2_2 = _separable_conv_block(act, p, filters, (7, 7), strides=(2, 2), weight_decay=weight_decay,
        ...
        with K.name_scope('block_3'):
            ...
            x3_2 = _separable_conv_block(act, p, filters, (5, 5), strides=(2, 2), weight_decay=weight_decay,
        ...
        with K.name_scope('block_5'):
            x5_1 = _separable_conv_block(act, x1, filters, (3, 3), weight_decay=weight_decay, id='reduction_left4_%s' % id)
...
def _add_auxiliary_head(act, x, classes, weight_decay):
    ...
    with K.name_scope('auxiliary_branch'):
        ...
        auxiliary_x = Activation(act)(auxiliary_x)
